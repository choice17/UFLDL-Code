...
[opttheta, cost, exitflag] = minFunc( @(theta) softICACost(theta, patches, params), opttheta, options); % Use x or xw
% reshape visualize weights
W = reshape(opttheta, params.numFeatures, params.n);
display_network(W');
%save RICAweight.mat W opttheta;
...
numClasses  = 10; % doing 5-class digit recognition
...
%%% YOUR CODE HERE %%%
theta2 = randTheta2;
theta2(:)=minFunc(@softmax_regression_vec, theta2(:), options, trainFeatures, trainLabels);
%%======================================================================
%% STEP 5: Testing 
% Compute Predictions on tran and test sets using softmaxPredict
% and softmaxModel
%%% YOUR CODE HERE %%%
train_pred = softmaxpredict(trainFeatures,theta2);
test_pred = softmaxpredict(testFeatures,theta2);
...
% even train for 10 labels, we still get 100% train accuracy with ~97% test
% accuracy
